...
VERSION   ?= $(call get_gh_version,0xERR0R/blocky)
...
MOUNTFLAGS := \
	...
	# -v $(CURDIR)/snippets:/config/snippets \
...
PORTFLAGS  := \
	...
	# -p 853:853 \
	# -p 443:443 \
	# # port 53 may conflict with systemd-resolved
	# # or use host network
	# --net=host \
...
OTHERFLAGS := \
	...
	# -e BLOCKY_CONFURL="https://0xerr0r.github.io/blocky/v0.25/config.yml" \
	# -e TZ=Asia/Kolkata \
	#
...
get_config: ## get default config from github
	curl \
		-o $(CURDIR)/root/defaults/config.yml \
		-SL https://0xerr0r.github.io/blocky/v$(VERSION)/config.yml
		# -SL https://raw.githubusercontent.com/0xERR0R/blocky/main/docs/config.yml
